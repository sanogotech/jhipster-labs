...
class PersonGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Person entity")
        ...
        .repeat(2) {
            ...
            .post("/person/api/people")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "surname":"SAMPLE_TEXT", "gender":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_person_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created person")
                .get("/person${new_person_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created person")
            .delete("/person${new_person_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
